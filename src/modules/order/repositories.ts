...
export const createOrderPaymentWithItems = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const paymentInfo = await initializeTransaction(userEmail, totalNGN, `${CONFIG_CLIENT_BASE_URL}/verifyPayment`, order.id) // order id is collected from metadata and used at transaction verification
